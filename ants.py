...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        Right now, if we attempt to put a second ant in a Place, the add_insect method of the Place class will immediately cause an error. 
        Change add_insect so that the Place contains the container ant and the container ant contains the other ant:

        If the Ant currently occupying this Place can contain the Ant we are trying to add, then simply tell it to do so.
        If the Ant we are trying to add can contain the Ant currently occupying this Place, then have it do so and set this 
        Place's ant to be the newly added Ant.
        If neither Ant can contain the other, then raise the same assertion error as before.
        """
        if insect.is_ant():
            # Phase 2: Special handling for BushAnt
            "*** YOUR CODE HERE ***"
            if self.ant != None and self.ant.container == True:
                self.ant.ant = insect
            elif self.ant != None and insect.container == True:
                insect.ant = self.ant
                self.ant = insect
            else:
...
class Ant(Insect):
    ...
    """We also need to give Ants a new method, can_contain, that takes an other ant as an argument and returns True if and only if:
    ...
    def can_contain(self, otherAnt):
        if self.container == True and otherAnt.container == False and self.ant == None:
            return True
        else:
            return False
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        "*** YOUR CODE HERE ***"
        if insect.is_ant() and insect.watersafe:
            # Phase 2: Special handling for BushAnt
            "*** YOUR CODE HERE ***"
            assert self.ant is None, 'Two ants in {0}'.format(self)
            self.ant = insect
            insect.place = self
        elif insect.watersafe:
            self.bees.append(insect)
            insect.place = self
        else:
            insect.armor = 0
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            ...
            for bee in self.place.bees:
                if bee.armor - self.fire_damage > 0:
                    newList.append(bee)
            self.place.bees = newList
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        "*** YOUR CODE HERE ***"
        temp = 0
        while temp <= 3 and self.place != None:
            self.place = self.place.entrance 
            temp = temp + 1

        while temp <= self.max_range and self.place != None:
            b = random_or_none(self.place.bees)
            if b != None and self.place != hive:
                return b
            else:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        "*** YOUR CODE HERE ***"
        """Eat a random Bee in its place (killing the Bee and restarting the digesting timer)."""
...
class BushAnt(Ant):
    ...
    def reduce_armor(self, amount):
        "*** YOUR CODE HERE ***"
        """When a BushAnt perishes, we need to make sure the ant it currently contains
        takes the BushAnt's place. Override the reduce_armor method so that, if the BushAnt perishes, 
        it will set its place's ant to be the ant it currently contains."""
        self.armor = self.armor - amount
...
def make_slow(action):
    ...
    """
    "*** YOUR CODE HERE ***"
    def slow_helper(duration):
        def slow_helper2(colony):
            nonlocal duration
            if duration > 0:
                duration = duration - 1
                if colony.time % 2 == 0:
...
def make_stun(action):
    ...
    """
    "*** YOUR CODE HERE ***"
    def stun_helper(duration):
        def stun_helper2(colony):
            nonlocal duration
            if duration > 0:
                duration = duration - 1
                return
...
def apply_effect(effect, bee, duration):
    ...
    apply_effect takes an effect (either make_slow or make_stun), a bee, and a duration. 
    It then takes the bee's original action along with the "affected action" 
    (the result of calling effect on the original action) and replaces the bee's action with 
    a new action method that will call the affected action for duration turns and then will go 
    back to calling the original action every turn. """
    "*** YOUR CODE HERE ***"
